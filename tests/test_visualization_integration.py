...
class TestVisualizationIntegration:
    ...
    def test_e2e_visualization_workflow(self, benchmark_results):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            fig3 = plot_rouge_breakdown(
                benchmark_results[0],
                output_path=rouge_path
            )
    ...
    def test_save_and_load_results(self, benchmark_results):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for i, (loaded, original) in enumerate(zip(loaded_results, benchmark_results)):
                assert loaded["summarizer_type"] == original["summarizer_type"]
                assert loaded["config"]["extractive_method"] == original["config"]["extractive_method"]
                ...
                for metric, value in original["overall_metrics"].items():
                    ...
                    assert loaded["overall_metrics"][metric] == pytest.approx(value, abs=1e-6)
