...
import llamasum
from llamasum.benchmark import load_dataset, save_results, run_benchmark
...
class TestBenchmarkFunctions:
    ...
    def test_load_dataset_json(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".json", mode="w", delete=False) as f:
            json.dump({
                "documents": [
                    {"id": "1", "text": "Sample text one.", "reference_summary": "Summary one."},
                    {"id": "2", "text": "Sample text two.", "reference_summary": "Summary two."}
                ]
            }, f)
            temp_path = f.name
...
class TestBenchmarkExecution:
    ...
    def test_run_benchmark_basic(self, sample_dataset_path):
        ...
        try:
            ...
            # Check metrics
            assert "compression_ratio" in results["document_results"]["test1"]["metrics"]
...
class TestBenchmarkIntegration:
    ...
    def test_benchmark_with_reference(self, sample_dataset_path):
        ...
        try:
            ...
            assert 0 <= similarity <= 1, f"Similarity {similarity} should be between 0 and 1"
        finally:
            # Clean up
            os.unlink(sample_dataset_path) 