...
from llamasum.preprocessing import remove_redundant_sentences, split_into_sentences
...
try:
    ...
    logger.warning(
        "sklearn not available, using basic similarity for multi-document summarization"
    )
...
class MultiDocSummarizer:
    ...
    def summarize(
        self, documents: List[str], max_length: Optional[int] = None, **kwargs
    ) -> Dict[str, Any]:
        ...
        if len(documents) == 1:
            summary = self.summarizer.summarize(
                documents[0], max_length=max_length or self.config.max_length, **kwargs
            )
        ...
        if strategy == "iterative":
            ...
            return self._summarize_extract_then_summarize(
                documents, max_length, **kwargs
            )

    def _summarize_iterative(
        self, documents: List[str], max_length: int, **kwargs
    ) -> Dict[str, Any]:
        ...
        for doc in documents:
            ...
            summary = self.summarizer.summarize(
                doc, max_length=doc_max_length, **kwargs
            )
        ...
        final_summary = self.summarizer.summarize(
            combined_text, max_length=max_length, **kwargs
        )
    ...
    def _summarize_extract_then_summarize(
        ...
        selected_indices = self._extract_important_sentences(
        ...
        final_summary = self.summarizer.summarize(
            extracted_text, max_length=max_length, **kwargs
        )
    ...
    def _summarize_hierarchical(
        ...
        if SKLEARN_AVAILABLE and len(documents) > 3:
            ...
            final_summary = self.summarizer.summarize(
                combined_summary, max_length=max_length, **kwargs
            )
        ...
        # Strategy 2 (fallback): Treat the combined documents as sections
            ...
            if len(final_summary.split()) > max_length:
                final_summary = self.summarizer.summarize(
                    final_summary, max_length=max_length, **kwargs
                )
    ...
    def _extract_important_sentences(
        self, sentences: List[str], num_to_extract: int
    ) -> List[int]:
        ...
        try:
            ...
            indices = list(
                range(0, len(sentences), max(1, len(sentences) // num_to_extract))
            )
            return indices[:num_to_extract]

    def _cluster_documents(self, documents: List[str]) -> List[List[str]]:
        ...
        try:
            ...
            num_clusters = min(self.config.num_clusters, max(2, len(documents) // 2))
    ...
    def summarize_with_queries(
        ...
        **kwargs,
        ...
        general_result = self.summarize(documents, max_length=max_length, **kwargs)
        ...
        for query in queries:
            ...
            query_summary = self.summarizer.summarize(
                query_content, max_length=max_length, **kwargs
            )
    ...
    def _extract_query_relevant_content(self, documents: List[str], query: str) -> str:
        ...
        try:
            ...
            num_to_select = min(
                len(all_sentences), max(20, int(len(all_sentences) * 0.3))
            )
