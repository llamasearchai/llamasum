...
class SummarizeRequest(BaseModel):
    ...
    ratio: Optional[float] = Field(
        None, description="Compression ratio for extractive phase (0.0-1.0)"
    )
    model_name: Optional[str] = Field(
        None, description="Model to use (overrides default config)"
    )
    device: Optional[str] = Field(None, description="Device to use (cpu, cuda, mps)")
...
class HierarchicalRequest(SummarizeRequest):
    ...
    levels: Optional[int] = Field(
        None, description="Number of summary levels to generate"
    )
    include_ultra_short: Optional[bool] = Field(
        None, description="Include ultra-short summary"
    )
    method: Optional[str] = Field(
        "standard", description="Summarization method (standard, sections, clustering)"
    )
...
class MultiDocRequest(BaseModel):
    ...
    strategy: Optional[str] = Field(
        None, description="Strategy for multi-document summarization"
    )
    model_name: Optional[str] = Field(
        None, description="Model to use (overrides default config)"
    )
    ...
    queries: Optional[List[str]] = Field(
        None, description="List of queries for query-focused summarization"
    )
...
def create_app() -> FastAPI:
    ...
    async def startup_event():
        ...
        try:
            ...
            logger.info(
                f"Initializing summarizers with model: {basic_config.model_name}"
            )
    ...
    async def summarize(request: SummarizeRequest):
        ...
        try:
            ...
            return {
                ...
                "compression_ratio": (
                    round(len(summary.split()) / len(request.text.split()), 2)
                    if len(request.text.split()) > 0
                    else 0
                ),
            }
    ...
    async def summarize_hierarchical(request: HierarchicalRequest):
        ...
        if not hierarchical_summarizer:
            raise HTTPException(
                status_code=500, detail="Hierarchical summarizer not initialized"
            )
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Hierarchical summarization error: {str(e)}"
            )
    ...
    async def summarize_multidoc(request: MultiDocRequest):
        ...
        if not multidoc_summarizer:
            raise HTTPException(
                status_code=500, detail="Multi-document summarizer not initialized"
            )
        ...
        try:
            ...
            if request.queries:
                result = summarizer.summarize_with_queries(
                    request.documents, request.queries, max_length=request.max_length
                )
            else:
                result = summarizer.summarize(
                    request.documents, max_length=request.max_length
                )
            ...
            raise HTTPException(
                status_code=500, detail=f"Multi-document summarization error: {str(e)}"
            )
...
def run_server(host: str = "0.0.0.0", port: int = 8000, reload: bool = False):
    ...
    """
    uvicorn.run("llamasum.api:app", host=host, port=port, reload=reload)
