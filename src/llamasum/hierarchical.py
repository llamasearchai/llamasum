...
from llamasum.preprocessing import (
    detect_sections,
...
try:
    ...
    logger.warning(
        "sklearn not available, using basic clustering for hierarchical summarization"
    )
...
class HierarchicalSummarizer:
    ...
    def summarize(
        self, text: str, levels: Optional[int] = None, **kwargs
    ) -> Dict[str, str]:
        ...
        for level in range(1, levels + 1):
            ...
            summary = self.summarizer.summarize(
                previous_summary, max_length=level_max_length, **kwargs
            )
    ...
    def summarize_by_sections(self, text: str, **kwargs) -> Dict[str, Any]:
        ...
        sections = detect_sections(
            text, min_section_length=self.config.min_section_length
        )
        ...
        for section in sections:
            ...
            summary = self.summarizer.summarize(
                section, max_length=section_max_length, **kwargs
            )
            section_summaries.append(summary)

        # Remove redundancies between section summaries
        unique_summaries = remove_redundant_sentences(
            section_summaries, similarity_threshold=self.config.redundancy_threshold
        )
        ...
        overall_summary = self.summarizer.summarize(
            combined_summary, max_length=overall_max_length, **kwargs
        )
    ...
    def summarize_by_clustering(
        self, text: str, num_clusters: Optional[int] = None, **kwargs
    ) -> Dict[str, Any]:
        ...
        if not SKLEARN_AVAILABLE:
            logger.warning(
                "sklearn not available, falling back to section-based summarization"
            )
        ...
        if num_clusters is None:
            ...
            num_clusters = min(
                self.config.max_clusters, max(2, int(math.sqrt(len(paragraphs)) / 2))
            )
        ...
        try:
            ...
            for cluster_id, contents in cluster_contents.items():
                ...
                # Summarize cluster
                summary = self.summarizer.summarize(
                    cluster_text, max_length=cluster_max_length, **kwargs
                )
                cluster_summaries.append(summary)

            # Remove redundancies between cluster summaries
            unique_summaries = remove_redundant_sentences(
                cluster_summaries, similarity_threshold=self.config.redundancy_threshold
            )
    ...
    def _generate_ultra_short(self, text: str) -> str:
        ...
        return self.summarizer.summarize(text, max_length=20, min_length=10)
