...
from llamasum.evaluation import calculate_compression_ratio, evaluate_summary
            ...
            try:
                ...
                if summary_type == "section-based":
                    ...
                    if "section_summaries" in result and result["section_summaries"]:
                        ...
                        for i, section_summary in enumerate(
        ...
        for i, doc in enumerate(st.session_state.documents):
            ...
            with col1:
                st.session_state.documents[i] = st.text_area(
                    f"Document {i+1}", value=doc, height=150, key=f"doc_{i}"
                )
            with col2:
    ...
    if run_button and any(doc.strip() for doc in st.session_state.documents):
        with st.spinner("Generating multi-document summary..."):
            try:
                ...
                with st.expander("Debug Information"):
                    if isinstance(result, dict):
                        debug_info = {
                            ...
                            if k not in ["summary", "query_summaries"]
                        }
                        st.json(debug_info)
                ...
                logger.error(
                    f"Error in multi-document summarization: {e}", exc_info=True
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--host", default="localhost", help="Host to run the web UI on")
    parser.add_argument(
        "--port", type=int, default=8501, help="Port to run the web UI on"
    )
