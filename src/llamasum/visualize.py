...
def plot_metrics_comparison(
    ...
    if metrics is None:
        available_metrics = [
            ...
            if col.startswith("avg_") and col != "avg_processing_time"
        ]
    ...
    for i, metric in enumerate(metrics):
        ...
        colors = {"basic": "blue", "hierarchical": "green", "multidoc": "orange"}
...
def plot_document_metrics(
    ...
    if metrics is None:
        ...
        if any(col.startswith("rouge_") for col in df.columns):
            rouge_metrics = [
                col
                for col in df.columns
                if col.endswith("_f") and col.startswith("rouge_")
            ]
    ...
    # Add overall title
    fig.suptitle(
        f"Document Metrics for {results.get('summarizer_type', 'Unknown').title()} Summarizer",
        fontsize=16,
    )
...
def plot_rouge_breakdown(
    ...
    top_docs = (
        df[df["rouge_type"] == "rouge-l"]
        .sort_values(by="f1", ascending=False)
        .head(max_docs)["document_id"]
        .tolist()
    )
...
def main():
    ...
    parser.add_argument(
        "-o", "--output", help="Directory to save output visualizations"
    )
    ...
    parser.add_argument(
        "--metrics", nargs="+", help="Specific metrics to visualize (comma-separated)"
    )

    parser.add_argument(
        "--figsize", default="12,8", help="Figure size in inches (width,height)"
    )
    ...
    if args.type in ["comparison", "all"] and len(results_list) > 1:
        ...
        plot_metrics_comparison(
            results_list, metrics=metrics, figsize=figsize, output_path=output_path
        )
        ...
        for i, results in enumerate(results_list):
            ...
            if args.output:
                ...
                output_path = os.path.join(
                    args.output, f"document_metrics_{summarizer_type}.{args.format}"
                )
        ...
        for i, results in enumerate(results_list):
            ...
            if has_rouge:
                ...
                if args.output:
                    summarizer_type = results.get("summarizer_type", f"result{i}")
                    output_path = os.path.join(
                        args.output, f"rouge_breakdown_{summarizer_type}.{args.format}"
                    )
