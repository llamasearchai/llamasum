...
from llamasum.config import SummarizerConfig, HierarchicalConfig, MultiDocConfig, load_config_from_env
...
def setup_summarizer(args) -> Union[LlamaSummarizer, HierarchicalSummarizer, MultiDocSummarizer]:
...
def main():
    ...
    input_group.add_argument(
        "files", nargs="*", help="Input file(s) to summarize"
    )
    input_group.add_argument(
    ...
    summary_group.add_argument(
        "--max-length", type=int, default=150,
    ...
    hierarchical_group.add_argument(
        ...
        help="Enable hierarchical summarization"
    )
    hierarchical_group.add_argument(
    ...
    hierarchical_group.add_argument(
        "--ultra-short", action="store_true",
        help="Include ultra-short (1-2 sentence) summary"
    )
    hierarchical_group.add_argument(
        "--by-sections", action="store_true",
        help="Summarize by detecting sections"
    )
    
    # Multi-document options
    multidoc_group = parser.add_argument_group("Multi-document Options")
    multidoc_group.add_argument(
        "--multidoc", action="store_true",
        help="Enable multi-document summarization"
    )
    multidoc_group.add_argument(
        "--strategy", choices=["extract_then_summarize", "iterative", "hierarchical"],
        default="extract_then_summarize",
        help="Strategy for multi-document summarization"
    )
    multidoc_group.add_argument(
        "--redundancy", type=float, default=0.8,
