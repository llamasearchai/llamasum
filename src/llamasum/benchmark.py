...
def load_dataset(dataset_path: str) -> List[Dict[str, str]]:
    ...
    if dataset_path.lower().endswith(".json"):
        ...
        if isinstance(data, dict) and "documents" in data:
            ...
            raise ValueError("Invalid JSON format. Expected list or object with 'documents' field.")
...
def run_benchmark(
    ...
    **kwargs
    ...
    for i, doc in enumerate(documents):
        ...
        if summarizer_type == "hierarchical":
            ...
            if "texts" in doc and isinstance(doc["texts"], list):
                ...
                if delimiter and delimiter in text:
                    multiple_texts = [t.strip() for t in text.split(delimiter) if t.strip()]
                    summary_result = summarizer.summarize(multiple_texts, **kwargs)
                else:
        ...
        if summarizer_type != "basic" and isinstance(summary_result, dict):
            results["document_results"][doc_id]["additional_data"] = {
                k: v for k, v in summary_result.items() 
    ...
    overall_metrics = {
        "avg_processing_time": sum(d["processing_time"] for d in results["document_results"].values()) / len(results["document_results"]),
        "avg_compression_ratio": sum(d["metrics"]["compression_ratio"] for d in results["document_results"].values()) / len(results["document_results"]),
        "avg_overall_score": sum(d["metrics"]["overall_score"] for d in results["document_results"].values()) / len(results["document_results"])
    }
    
    # Check if all have ROUGE scores
    rouge_available = all("rouge" in d["metrics"] for d in results["document_results"].values())
    if rouge_available:
        overall_metrics["avg_rouge_1_f"] = sum(d["metrics"]["rouge"]["rouge-1"]["f"] for d in results["document_results"].values()) / len(results["document_results"])
        overall_metrics["avg_rouge_l_f"] = sum(d["metrics"]["rouge"]["rouge-l"]["f"] for d in results["document_results"].values()) / len(results["document_results"])
...
def main():
    ...
    parser.add_argument(
        "--strategy",
        choices=["concatenate", "summarize_each", "extract_then_summarize", "cluster_then_summarize"],
