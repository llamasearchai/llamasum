    ...
    def summarize(
        ...
        **kwargs,
        ...
        if len(sentences) > self.config.extractive_threshold:
            ...
            extraction_method = kwargs.get(
                "extraction_method", self.config.extraction_method
            )
            key_sentences = extract_key_sentences(
                sentences, extraction_ratio=extractive_ratio, method=extraction_method
            )
        ...
        if TRANSFORMERS_AVAILABLE and self.model and self.tokenizer:
            return self._generate_abstractive_summary(
                extracted_text, max_length=max_length, min_length=min_length, **kwargs
            )
        else:
            ...
            return " ".join(
                extract_key_sentences(
                    sentences,
                    extraction_ratio=min(
                        0.3, max_length / len(sentences) if len(sentences) > 0 else 0.3
                    ),
                )
            )

    def _generate_abstractive_summary(
        self, text: str, max_length: int, min_length: int, **kwargs
    ) -> str:
        ...
        try:
            ...
            gen_kwargs.update({k: v for k, v in kwargs.items() if k in gen_kwargs})
            ...
            return " ".join(
                extract_key_sentences(
                    ...
                    extraction_ratio=min(
                        0.3, max_length / len(text.split()) if text else 0.3
                    ),
                )
            )
