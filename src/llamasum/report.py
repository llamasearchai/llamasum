...
def generate_report_html(
    ...
    if not MATPLOTLIB_AVAILABLE or not PANDAS_AVAILABLE:
        logger.error("Visualization dependencies not available. Install with: pip install llamasum[viz]")
    ...
    if comparison_b64:
        html += """        <div class="viz-section">
            ...
            <div class="viz-container">
                <img src="data:image/png;base64,""" + comparison_b64 + """" alt="Metrics Comparison">
            </div>
        </div>
"""
...
"""
        ...
        doc_viz = next((b64 for name, b64 in document_b64 if name == summarizer_type), None)
        if doc_viz:
...
"""
        ...
        rouge_viz = next((b64 for name, b64 in rouge_b64 if name == summarizer_type), None)
        if rouge_viz:
            html += f"""            <div class="viz-container">
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate HTML reports from LlamaSum benchmark results")
    ...
    parser.add_argument(
        "--title",
        default="LlamaSum Benchmark Report",
