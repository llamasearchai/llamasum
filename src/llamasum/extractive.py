...
def extract_key_sentences(
    ...
    num_sentences = max(
        min_sentences, min(max_sentences, int(len(sentences) * extraction_ratio))
    )
...
def extract_by_position(sentences: List[str], num_sentences: int) -> List[str]:
    ...
    if remaining > 0:
        ...
        while len(positions) < num_sentences and len(positions) < len(sentences):
            ...
            if (
                midpoint not in positions
                and midpoint > 0
                and midpoint < len(sentences) - 1
            ):
...
def extract_keywords(text: str, num_keywords: int = 10, min_freq: int = 2) -> List[str]:
    ...
    word_counts = {
        word: count for word, count in word_counts.items() if count >= min_freq
    }
