...
try:
    from llamasum.benchmark import run_benchmark, load_dataset
...
def run_comparative_benchmarks(output_dir):
    ...
    configs = [
        ...
        # Multi-document summarizer
        {
            "name": "multidoc",
            "type": "multidoc",
            "strategy": "cluster_then_summarize"
        }
    ]
...
def create_interactive_comparison(results_list, output_dir):
        ...
        for doc_id, doc_data in result.get("document_results", {}).items():
            if "metrics" in doc_data:
                doc_rows.append({
                    ...
                    **{k: v for k, v in doc_data["metrics"].items() if not isinstance(v, dict)}
                })
    ...
    metrics = ["avg_overall_score", "avg_rouge_1_f", "avg_rouge_l_f", 
              "avg_compression_ratio", "avg_similarity_to_reference"]
    ...
    if metrics:
        ...
        for i, row in df_radar.iterrows():
            ...
            ax.plot(angles, values, linewidth=2, linestyle='solid', label=f"{row['model']}_{row['method']}")
    ...
    if all(col in df_results.columns for col in ["avg_processing_time", "avg_overall_score"]):
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Advanced LlamaSum visualization example")
    ...
    if not VISUALIZATION_AVAILABLE:
        print("Warning: Full visualization capabilities require additional dependencies.")
    ...
    for result in results_list:
        ...
        plot_document_metrics(
            result,
            output_path=str(output_dir / f"document_metrics_{name}.png")
        )
        ...
        if has_rouge:
            plot_rouge_breakdown(
                result,
                output_path=str(output_dir / f"rouge_breakdown_{name}.png")
            )
